...
class Dashboard extends CI_Controller {
	...
	public function form_a(){
		$data = array(
			'title' => 'Realisasi Penerimaan VS Target PNBP BLU ('.date('d F Y').')-Form A' ,
			'page'	=> 'form_a',
        ...
        foreach($highLevel as $row){
            ...
            if (strtolower($row['jenis']) == 'realisasi'){
				$namaSeries = $row['tahun'].' '.$row['jenis'];
                if(!in_array($namaSeries, $arrNamaSeries)) {
					$arrNamaSeries[] = $namaSeries;
				}
                
                $dataSeries[$namaSeries][] = floatval(number_format($row['nilai'],2));
            }
            else if (strtolower($row['jenis']) == 'target'){
				$namaSeries = $row['tahun'].' '.$row['jenis'];
                if(!in_array(ucfirst($namaSeries), $arrNamaSeries)){
                    $arrNamaSeries[] = $namaSeries;
				}
